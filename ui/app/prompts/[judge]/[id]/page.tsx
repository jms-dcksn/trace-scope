...
import { cloneAsNewVersion, savePrompt, setActive } from "@/app/prompts/actions";
...
export default async function PromptDetail({
  ...
  params: Promise<{ judge: string; id: string }>;
}) {
  const { judge, id } = await params;
  const promptId = Number(id);
  const p = getPrompt(promptId);
  if (!p || p.judge_name !== judge) notFound();
  const placeholders = PLACEHOLDERS[judge] ?? [];
  const usedBy = promptUsedByRunCount(judge, p.version);

  return (
    <div className="space-y-6">
      <div>
        <Link href={`/prompts/${judge}`} className="text-sm text-blue-600 hover:underline">
          ← {judge}
        </Link>
        ...
        {usedBy > 0 && (
          <p className="text-xs text-amber-600 mt-1">
            ⚠ This version has been used by stored runs. Editing will retroactively change what those
            runs claim to have used. Consider “Clone to new version” instead.
          </p>
        )}
      </div>
      ...
      {placeholders.length > 0 && (
        <section className="text-sm">
          <div className="text-zinc-500 mb-1">Placeholders available in this template:</div>
          <div className="flex flex-wrap gap-2">
            {placeholders.map((ph) => (
              <code key={ph} className="px-2 py-1 bg-zinc-100 dark:bg-zinc-900 rounded text-xs">
                {`{${ph}}`}
              </code>
            ))}
          </div>
      ...
      <section className="grid grid-cols-2 gap-4">
        {!p.is_active && (
          <form action={setActive} className="border border-zinc-200 dark:border-zinc-800 rounded p-3 space-y-2">
            ...
            <p className="text-xs text-zinc-500">
              Mark this version active. Subsequent judge calls will load this template.
            </p>
        ...
        <form action={cloneAsNewVersion} className="border border-zinc-200 dark:border-zinc-800 rounded p-3 space-y-2">
          ...
          <input
            ...
            className="w-full p-2 text-sm border border-zinc-300 dark:border-zinc-700 rounded bg-white dark:bg-zinc-950"
          />
          <button className="px-3 py-1.5 text-sm bg-zinc-700 text-white rounded hover:bg-zinc-800">
            Create new version
          </button>
        </form>
      </section>
